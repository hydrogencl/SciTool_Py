...
class MATH_TOOLS:
    ...
    def x_exp_fix(x,x_min,x_max,sigma=1.0):
        return math.exp(-1.0*(x**2-x_min**2)/(x_max-x_min) )

    def expontial_prob(NUM_IN, NUM_TOTAL=0):
        if NUM_TOTAL == 0:
            return 1 - math.exp(-0.5 * ( NUM_IN ) )
        else:
            return 1 - math.exp(-0.5 * ( NUM_IN / NUM_TOTAL ) )
...
class WRF_HELPER:
    ...
    def wrf_time2array(STR_IN="", WRF_IN=None, WRF_TIMESTEP=9999999):
        if WRF_IN != None:
            STR_IN = str(NC.chartostring(WRF_IN.variables["Times"][WRF_TIMESTEP]))
        try:
            STR_DATE, STR_TIME = re.split("_", STR_IN)
            ARR_DATE = re.split("-", STR_DATE)
            ARR_TIME = re.split(":", STR_TIME)
            return [int(ARR_DATE[0]), int(ARR_DATE[1]), int(ARR_DATE[2]),\
                    int(ARR_TIME[0]), int(ARR_TIME[1]), int(ARR_TIME[2])]
        except:
            print(" Error: wrf_time2array. Did you 1) input WRF_IN & WRF_TIME_STEP? 2) input STR_IN?")
            return None
...
class DATA_IO:
    ...
    """
    def __init__(self, STR_NULL="noData", NUM_NULL=-999.999, IF_DEBUG=False, STR_endian="b"):
